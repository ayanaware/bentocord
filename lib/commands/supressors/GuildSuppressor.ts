...
export class GuildSuppressor implements Suppressor {
	...
	public async suppress(ctx: CommandContext, option: SuppressorOption, guildIds?: Array<string>): Promise<string | false> {
		if (!ctx.guildId) return await ctx.getTranslation('BENTOCORD_SUPPRESSOR_GUILD') || 'This command must be executed in a server.';

		if (!Array.isArray(guildIds)) return false;

		const message = await ctx.getTranslation('BENTOCORD_SUPPRESSOR_GUILD_DENIED') || 'Server is not allowed to execute this command.';
