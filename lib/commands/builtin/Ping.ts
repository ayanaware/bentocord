...
export class PingCommand implements CommandEntity {
	...
	public async execute(ctx: CommandContext): Promise<unknown> {
		...
		await ctx.createResponse(await ctx.getTranslation('BENTOCORD_PING') || 'Pong!');
		const end = process.hrtime(start);

		const s = end[0];
		const ms = end[1] / 1000000;

		return ctx.editResponse(await ctx.getTranslation('BENTOCORD_PING_COUNT', { s, ms }) || `Pong! \`${s}s ${ms}ms\``);
	}
}
