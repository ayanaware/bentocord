...
export class GuildOptionResolver implements Resolver<Guild> {
	...
	public async reduce?(ctx: AnyCommandContext, option: GuildOption, guild: Guild): Promise<{ display: string, extra?: string }> {
		return { display: guild.name, extra: guild.id };
	}

	public async resolve(ctx: AnyCommandContext, option: GuildOption, input: string): Promise<Array<Guild>> {
